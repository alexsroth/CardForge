...
const TAILWIND_BORDER_RADIUS_OPTIONS = [
...
const TAILWIND_BORDER_COLOR_OPTIONS = [
...
const BORDER_SIDE_WIDTH_OPTIONS: { value: string; label: string; classPrefix: 'border-t' | 'border-r' | 'border-b' | 'border-l' }[] = [
  { value: NONE_VALUE, label: "None", classPrefix: 'border-t'}, // classPrefix is arbitrary for NONE_VALUE
  { value: 'default', label: "Default (1px)", classPrefix: 'border-t' }, // Mapped to 'border-t', 'border-r' etc.
  { value: '0', label: "0px", classPrefix: 'border-t' }, // Mapped to 'border-t-0'
  { value: '2', label: "2px", classPrefix: 'border-t' },
  { value: '4', label: "4px", classPrefix: 'border-t' },
  { value: '8', label: "8px", classPrefix: 'border-t' },
];
...
export interface LayoutElementGuiConfig {
  ...
  styleBorderTop?: string; styleBorderBottom?: string;
...
export default function TemplateDesignerPage() {
  ...
  const [canvasWidthSetting, setCanvasWidthSetting] = useState<string>(String(defaultLayout.width || `${DEFAULT_CANVAS_WIDTH}px`));
  const [canvasHeightSetting, setCanvasHeightSetting] = useState<string>(String(defaultLayout.height || `${DEFAULT_CANVAS_HEIGHT}px`));
  ...
  const [canvasBorderStyle, setCanvasBorderStyle] = useState<string>(defaultLayout.borderStyle || "solid");
  ...
  const populateGuiFromLayoutDefinition = useCallback(() => {
    console.log('[DEBUG] NewTemplatePage/populateGuiFromLayoutDefinition: Parsing layoutDefinition to populate GUI state.');
    let parsedLayout: LayoutDefinition;
    try {
      parsedLayout = JSON.parse(layoutDefinition || DEFAULT_CARD_LAYOUT_JSON_STRING);
    } catch (e) {
      console.warn("[DEBUG] NewTemplatePage: Error parsing layoutDefinition for GUI population, using defaults.", e);
      parsedLayout = defaultLayout;
    }

    setCanvasWidthSetting(parsedLayout.width || defaultLayout.width || `${DEFAULT_CANVAS_WIDTH}px`);
    setCanvasHeightSetting(parsedLayout.height || defaultLayout.height || `${DEFAULT_CANVAS_HEIGHT}px`);
    setCanvasBorderStyle(parsedLayout.borderStyle || defaultLayout.borderStyle || "solid");

    const canvasClasses = (parsedLayout.canvasClassName || '').split(' ').filter(Boolean);
    setTailwindCanvasBackgroundColor(canvasClasses.find(c => TAILWIND_TEXT_COLORS.some(opt => opt.value === c.replace("bg-","text-")))?.replace("text-","bg-") || NONE_VALUE);
    setTailwindCanvasBorderRadius(canvasClasses.find(c => TAILWIND_BORDER_RADIUS_OPTIONS.some(opt => opt.value === c)) || NONE_VALUE);
    setTailwindCanvasBorderColor(canvasClasses.find(c => TAILWIND_BORDER_COLOR_OPTIONS.some(opt => opt.value === c)) || NONE_VALUE);
    setTailwindCanvasBorderWidth(canvasClasses.find(c => ["border", "border-0", "border-2", "border-4", "border-8"].includes(c)) || NONE_VALUE);

    const elementsFromJsonMap = new Map((parsedLayout.elements || []).map(el => [el.fieldKey, el]));
    const newGuiConfigs = fields.map((field, index) => {
      const existingConfig = layoutElementGuiConfigs.find(lc => lc._uiId === field._uiId);
      const existingLayoutElement = elementsFromJsonMap.get(field.key);
      const isEnabled = !!existingLayoutElement;
      const defaultTop = `${10 + index * 25}px`; // Simple cascade for new elements
      ...
      const config: LayoutElementGuiConfig = {
        ...
        isExpandedInGui: existingConfig?.isExpandedInGui || false,
        elementType: existingLayoutElement?.type || 'text',
        iconName: existingLayoutElement?.icon || (field.type === 'number' ? 'Coins' : ''),
        styleTop: existingLayoutElement?.style?.top ?? (isEnabled ? '' : defaultTop),
        styleLeft: existingLayoutElement?.style?.left ?? (isEnabled ? '' : '10px'),
        styleRight: existingLayoutElement?.style?.right ?? '',
        styleBottom: existingLayoutElement?.style?.bottom ?? '',
        styleMaxHeight: existingLayoutElement?.style?.maxHeight ?? (field.type === 'textarea' ? '80px' : 'auto'),
        ...
        tailwindTextColor: TAILWIND_TEXT_COLORS.find(opt => (existingLayoutElement?.className || '').includes(opt.value) && opt.value !== NONE_VALUE)?.value || "text-black",
        tailwindFontSize: TAILWIND_FONT_SIZES.find(opt => (existingLayoutElement?.className || '').includes(opt.value) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
        tailwindFontWeight: TAILWIND_FONT_WEIGHTS.find(opt => (existingLayoutElement?.className || '').includes(opt.value) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
        tailwindLineHeight: TAILWIND_LINE_HEIGHTS.find(opt => (existingLayoutElement?.className || '').includes(opt.value) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
        tailwindOverflow: TAILWIND_OVERFLOW.find(opt => (existingLayoutElement?.className || '').includes(opt.value) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
        tailwindTextOverflow: TAILWIND_TEXT_OVERFLOW.find(opt => (existingLayoutElement?.className || '').includes(opt.value) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
        tailwindBorderRadius: TAILWIND_BORDER_RADIUS_OPTIONS.find(opt => (existingLayoutElement?.className || '').includes(opt.value) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
        tailwindBorderColor: TAILWIND_BORDER_COLOR_OPTIONS.find(opt => (existingLayoutElement?.className || '').includes(opt.value) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
        tailwindBorderTopW: BORDER_SIDE_WIDTH_OPTIONS.find(opt => (existingLayoutElement?.className || '').includes(getSideBorderWidthClass('t', opt.value)) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
        tailwindBorderRightW: BORDER_SIDE_WIDTH_OPTIONS.find(opt => (existingLayoutElement?.className || '').includes(getSideBorderWidthClass('r', opt.value)) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
        tailwindBorderBottomW: BORDER_SIDE_WIDTH_OPTIONS.find(opt => (existingLayoutElement?.className || '').includes(getSideBorderWidthClass('b', opt.value)) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
        tailwindBorderLeftW: BORDER_SIDE_WIDTH_OPTIONS.find(opt => (existingLayoutElement?.className || '').includes(getSideBorderWidthClass('l', opt.value)) && opt.value !== NONE_VALUE)?.value || NONE_VALUE,
      };
      return config;
    });
    setLayoutElementGuiConfigs(newGuiConfigs);
  }, [fields, layoutDefinition, layoutElementGuiConfigs]); // Added layoutElementGuiConfigs to ensure existing expanded states are preserved

  useEffect(() => {
    if (templateName.trim()) { setTemplateId(toCamelCase(templateName.trim())); } 
    else { setTemplateId(''); }
  }, [templateName]);

  const handleGenerateJsonFromBuilder = useCallback(() => {
    console.log('[DEBUG] NewTemplatePage/handleGenerateJsonFromBuilder: Generating JSON from GUI.');
    ...
    const canvasClasses = [
      tailwindCanvasBackgroundColor !== NONE_VALUE ? tailwindCanvasBackgroundColor : '',
      tailwindCanvasBorderRadius !== NONE_VALUE ? tailwindCanvasBorderRadius : '',
      tailwindCanvasBorderWidth !== NONE_VALUE ? tailwindCanvasBorderWidth : '',
      (tailwindCanvasBorderWidth !== NONE_VALUE && tailwindCanvasBorderColor !== NONE_VALUE) ? tailwindCanvasBorderColor : (tailwindCanvasBorderWidth !== NONE_VALUE ? 'border-border' : ''),
    ].filter(Boolean).join(' ');
    ...
    const newLayout: LayoutDefinition = {
      ...
      borderStyle: canvasBorderStyle || defaultLayout.borderStyle || "solid",
      canvasClassName: canvasClasses.trim() || undefined,
    ...
    toast({ title: "Layout JSON Updated", description: "JSON generated from GUI builder and updated in textarea and preview."});
      ...
      tailwindCanvasBackgroundColor, tailwindCanvasBorderRadius, tailwindCanvasBorderColor, tailwindCanvasBorderWidth, canvasBorderStyle,
      toast
  ]);

  // Effect to update JSON from GUI when GUI elements change
  useEffect(() => {
    if (activeEditorView === 'gui') {
      // Simple debounce to avoid excessive updates
      const handler = setTimeout(() => {
        if (Date.now() - jsonEditorLastUpdateRef.current > 100) { // Prevent loop if JSON just updated GUI
          handleGenerateJsonFromBuilder();
        }
      }, 300); // Adjust debounce time as needed
      return () => clearTimeout(handler);
    }
  }, [layoutElementGuiConfigs, canvasWidthSetting, canvasHeightSetting, tailwindCanvasBackgroundColor, tailwindCanvasBorderRadius, tailwindCanvasBorderColor, tailwindCanvasBorderWidth, canvasBorderStyle, activeEditorView, handleGenerateJsonFromBuilder]);

  // Effect to update GUI from JSON when JSON textarea changes (and focus is lost, or view switches)
  useEffect(() => {
    if (activeEditorView === 'json' || (activeEditorView === 'gui' && Date.now() - guiBuilderLastUpdateRef.current > 100)) {
      populateGuiFromLayoutDefinition();
    }
  }, [layoutDefinition, activeEditorView, populateGuiFromLayoutDefinition]);


  useEffect(() => {
    const currentTemplateIdForPreview = templateId || 'previewTemplateId';
    const generatedSampleCard: Partial<CardData> = { id: 'preview-card', templateId: currentTemplateIdForPreview as CardTemplateId, };
    fields.forEach(fieldDef => {
      ...
      if (fieldDef.type === 'placeholderImage') {
        ...
        } else { valueForPreview = generateSamplePlaceholderUrl({ width: fieldDef.placeholderConfigWidth, height: fieldDef.placeholderConfigHeight, bgColor: fieldDef.placeholderConfigBgColor, textColor: fieldDef.placeholderConfigTextColor, text: fieldDef.placeholderConfigText }); }
    ...
    if (!fields.some(f => f.key === 'name')) generatedSampleCard.name = 'Awesome Card Name';
    if (!fields.some(f => f.key === 'cost')) generatedSampleCard.cost = 3;
    if (!fields.some(f => f.key === 'imageUrl')) generatedSampleCard.imageUrl = generateSamplePlaceholderUrl({width: parseInt(String(canvasWidthSetting).replace('px','')) || DEFAULT_CANVAS_WIDTH, height: 140, text: 'Main Image'});
    if (!fields.some(f => f.key === 'dataAiHint')) generatedSampleCard.dataAiHint = 'card art sample';
    if (!fields.some(f => f.key === 'cardType')) generatedSampleCard.cardType = 'Creature - Goblin';
    if (!fields.some(f => f.key === 'effectText')) generatedSampleCard.effectText = 'Sample effect: Draw a card.';
    if (!fields.some(f => f.key === 'attack')) generatedSampleCard.attack = 2;
    if (!fields.some(f => f.key === 'defense')) generatedSampleCard.defense = 2;
    if (!fields.some(f => f.key === 'artworkUrl')) generatedSampleCard.artworkUrl = generateSamplePlaceholderUrl({width: parseInt(String(canvasWidthSetting).replace('px','')) || DEFAULT_CANVAS_WIDTH, height: parseInt(String(canvasHeightSetting).replace('px','')) || DEFAULT_CANVAS_HEIGHT, text: 'Background Art'});
    if (!fields.some(f => f.key === 'statusIcon')) generatedSampleCard.statusIcon = 'ShieldCheck';
  ...
  const handleAddField = useCallback(() => {
    ...
    const newFieldDef: TemplateFieldDefinition = {
      _uiId: newUiId, key: newKey, label: newFieldLabel, type: 'text', placeholder: '', defaultValue: '', previewValue: '',
      optionsString: '', placeholderConfigWidth: parseInt(String(canvasWidthSetting).replace('px','')) || DEFAULT_CANVAS_WIDTH, placeholderConfigHeight: 140,
    };
    setFields(prevFields => [...prevFields, newFieldDef]);
  ...
  const handleRemoveField = useCallback((uiIdToRemove: string) => {
    ...
    if (fieldToRemove) {
      setLayoutElementGuiConfigs(prevConfigs => prevConfigs.filter(c => c._uiId !== fieldToRemove._uiId));
    }
  }, [fields]);
      ...
      prevFields.map(field => {
        if (field._uiId === uiIdToUpdate) {
          ...
          if (updatedFieldDefinition.label !== undefined && updatedFieldDefinition.label !== oldField.label) {
            ...
            if (!baseKey) { const prefix = 'field'; let fallbackCounter = 1; let potentialKey = `${prefix}${fallbackCounter}`;
              while (prevFields.some(f => f._uiId !== uiIdToUpdate && f.key === potentialKey)) { fallbackCounter++; potentialKey = `${prefix}${fallbackCounter}`; }
              baseKey = potentialKey;
            }
            let newKey = baseKey; let keyCounter = 1;
            while (prevFields.some(f => f._uiId !== uiIdToUpdate && f.key === newKey)) { newKey = `${baseKey}${keyCounter}`; keyCounter++; }
          ...
          if (updatedFieldDefinition.type === 'placeholderImage' && oldField.type !== 'placeholderImage') {
            ...
            modifiedField.placeholderConfigBgColor = undefined; modifiedField.placeholderConfigTextColor = undefined; modifiedField.placeholderConfigText = undefined;
          }
  ...
  const handleLayoutDefinitionChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    console.log('[DEBUG] NewTemplatePage/handleLayoutDefinitionChange: Layout string changed.');
  ...
  const handleSaveTemplate = async () => {
    ...
    if (!finalTemplateId) { toast({ title: "Invalid Name", description: "Template Name generates an empty ID. Provide valid name.", variant: "destructive" }); return; }
    if (fields.length === 0) { toast({ title: "No Fields", description: "Add at least one field.", variant: "destructive" }); return; }
    if (existingTemplates.some(t => t.id === finalTemplateId)) {
        toast({ title: "Duplicate ID", description: `Template ID '${finalTemplateId}' (from name '${templateName}') already exists. Choose unique name.`, variant: "destructive", }); return;
    }
    const fieldKeys = fields.map(f => f.key);
    const duplicateFieldKeys = fieldKeys.filter((key, index) => fieldKeys.indexOf(key) !== index);
    if (duplicateFieldKeys.length > 0) {
        toast({ title: "Duplicate Field Keys", description: `Keys must be unique. Duplicates: ${duplicateFieldKeys.join(', ')}. Adjust labels.`, variant: "destructive", }); return;
    }
    ...
    try { JSON.parse(finalLayoutDefToSave); } 
    catch (e: any) {
      setLayoutJsonError(`Invalid JSON: ${e.message}`);
      toast({ title: "Invalid Layout JSON", description: `JSON Error: ${e.message}. Correct or regenerate from builder.`, variant: "destructive", duration: 7000, });
      setActiveEditorView('json'); // Switch to JSON view if error
      return;
    }
    setIsSaving(true);
    const newTemplate: CardTemplate = {
      id: finalTemplateId as CardTemplateId, name: templateName.trim(), fields: fields.map(mapFieldDefinitionToTemplateField), layoutDefinition: finalLayoutDefToSave,
    };
    ...
    if (result.success) {
      toast({ title: "Template Saved!", description: result.message, variant: "default", duration: 7000, });
  ...
  const handleSizePresetChange = (value: string) => {
    ...
    if (value === "custom") { /* No change if custom */ } 
    else {
  ...
  const handleCustomDimensionChange = (dimension: 'width' | 'height', value: string) => {
    if (dimension === 'width') { setCanvasWidthSetting(value); } 
    else { setCanvasHeightSetting(value); }
    if (selectedSizePreset !== "custom") { setSelectedSizePreset("custom"); }
  };

  const handleCanvasTailwindChange = ( prop: 'tailwindCanvasBackgroundColor' | 'tailwindCanvasBorderRadius' | 'tailwindCanvasBorderColor' | 'tailwindCanvasBorderWidth', value: string) => {
    switch(prop) {
  ...
  useEffect(() => {
    setLayoutElementGuiConfigs(prevConfigs => {
      ...
      const newConfigs: LayoutElementGuiConfig[] = fields.map((field, index) => {
        ...
        if (config) {
          return { ...config, _uiId: field._uiId!, fieldKey: field.key, label: field.label, originalType: field.type, };
        } else {
          return {
            _uiId: field._uiId || `gui-cfg-new-${field.key}-${Date.now()}-${index}`,
            fieldKey: field.key, label: field.label, originalType: field.type, isEnabledOnCanvas: false, isExpandedInGui: false,
            ...
            stylePadding: '', styleFontStyle: 'normal', styleTextAlign: 'left', styleBorderTop: '', styleBorderBottom: '',
        ...
        <CardContent className="pt-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input id="templateName" value={templateName} onChange={(e) => setTemplateName(e.target.value)} placeholder="e.g., 'Hero Unit Card'" disabled={isSaving} className="mt-1" />
            </div>
            <div>
              <Label htmlFor="templateId" className="font-semibold">Template ID (auto-generated)</Label>
              <Input id="templateId" value={templateId} readOnly className="mt-1 bg-muted/50" disabled={isSaving || true} />
            </div>
          </div>
          <div>
            ...
            <div className="space-y-3">
                {fields.map((field) => (
                <FieldRow key={field._uiId} field={field} onChange={(updatedField) => handleFieldChange(field._uiId!, updatedField)} onRemove={() => handleRemoveField(field._uiId!)} isSaving={isSaving} />
                ))}
             ...
             <Button onClick={handleAddField} variant="outline" size="sm" disabled={isSaving} className="mt-4" title={"Add a new data field"}>
      ...
      <div className="md:flex md:flex-row md:gap-6 items-start">
          ...
          <CardHeader>
            ...
            <CardDescription className="text-sm">
              {activeEditorView === 'gui' 
                ? "Use the GUI below to configure card canvas properties and individual layout elements. Changes here will update the JSON output and preview in real-time."
                : "Directly edit the Layout Definition JSON. Changes here will update the preview in real-time and will be reflected in the GUI builder if you switch back."}
            </CardDescription>
          </CardHeader>
            ...
            {activeEditorView === 'gui' && (
              <>
                ...
                <div className="space-y-3 p-3 border rounded-md bg-muted/30">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3 items-end mt-2">
                    <div>
                        ...
                        <Select value={tailwindCanvasBackgroundColor} onValueChange={(val) => handleCanvasTailwindChange('tailwindCanvasBackgroundColor', val)} disabled={isSaving}>
                            <SelectTrigger id="tailwindCanvasBgColor" className="mt-1 h-8 text-xs"><SelectValue placeholder="Select color" /></SelectTrigger>
                            <SelectContent>{TAILWIND_TEXT_COLORS.map(opt => (<SelectItem key={opt.value} value={opt.value.replace("text-","bg-")}>{opt.label.replace("Text","BG")}</SelectItem>))}</SelectContent>
                        </Select>
                    </div>
                    <div>
                        <Label htmlFor="tailwindCanvasBorderRadius" className="text-xs font-medium">Border Radius (Tailwind)</Label>
                        <Select value={tailwindCanvasBorderRadius} onValueChange={(val) => handleCanvasTailwindChange('tailwindCanvasBorderRadius', val)} disabled={isSaving}>
                            <SelectTrigger id="tailwindCanvasBorderRadius" className="mt-1 h-8 text-xs"><SelectValue placeholder="Select radius" /></SelectTrigger>
                    ...
                    <div>
                        ...
                        <Select value={tailwindCanvasBorderWidth} onValueChange={(val) => handleCanvasTailwindChange('tailwindCanvasBorderWidth', val)} disabled={isSaving}>
                            <SelectTrigger id="tailwindCanvasBorderWidth" className="mt-1 h-8 text-xs"><SelectValue placeholder="Select width" /></SelectTrigger>
                            <SelectContent>{[{value: NONE_VALUE, label:"None"}, {value: 'border', label: '1px'}, {value: 'border-0', label: '0px'}, {value: 'border-2', label: '2px'}, {value: 'border-4', label: '4px'}, {value: 'border-8', label: '8px'}].map(opt => (<SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>))}</SelectContent>
                        </Select>
                    </div>
                    <div>
                        <Label htmlFor="tailwindCanvasBorderColor" className="text-xs font-medium">Border Color (Tailwind)</Label>
                        <Select value={tailwindCanvasBorderColor} onValueChange={(val) => handleCanvasTailwindChange('tailwindCanvasBorderColor', val)} disabled={isSaving}>
                            <SelectTrigger id="tailwindCanvasBorderColor" className="mt-1 h-8 text-xs"><SelectValue placeholder="Select color" /></SelectTrigger>
                            <SelectContent>{TAILWIND_BORDER_COLOR_OPTIONS.map(opt => (<SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>))}</SelectContent>
                        </Select>
                    </div>
                    <div>
                ...
                <div className="space-y-3 p-3 border rounded-md bg-muted/30">
                  ...
                  <ScrollArea className="pr-2"> {/* No max-h, will grow with content */}
                      <div className="space-y-2">
                        {layoutElementGuiConfigs.map((config) => (
                          <div key={config._uiId} className="p-2.5 border rounded-md bg-card/80 hover:bg-card transition-colors">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <Switch id={`enable-${config._uiId}`} checked={config.isEnabledOnCanvas} onCheckedChange={(checked) => handleGuiConfigChange(config._uiId, 'isEnabledOnCanvas', checked)} disabled={isSaving} />
                                <Label htmlFor={`enable-${config._uiId}`} className="text-sm font-medium cursor-pointer">{config.label} <span className="text-xs text-muted-foreground">({config.fieldKey})</span></Label>
                              </div>
                              ...
                              <div className="mt-3 pt-3 border-t border-dashed space-y-4">
                                ...
                                {(config.elementType === 'text' || config.elementType === 'textarea' || config.elementType === 'iconValue') && (
                                  <details className="space-y-2 group" open>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pl-1">
                                      <div><Label htmlFor={`el-twTextColor-${config._uiId}`} className="text-xs">Text Color (TW)</Label><Select value={config.tailwindTextColor || "text-black"} onValueChange={(value) => handleGuiConfigChange(config._uiId, 'tailwindTextColor', value)} disabled={isSaving}><SelectTrigger id={`el-twTextColor-${config._uiId}`} className="h-8 text-xs mt-0.5"><SelectValue placeholder="Select color" /></SelectTrigger><SelectContent>{TAILWIND_TEXT_COLORS.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent></Select></div>
                                      <div><Label htmlFor={`el-twFontSize-${config._uiId}`} className="text-xs">Font Size (TW)</Label><Select value={config.tailwindFontSize || NONE_VALUE} onValueChange={(value) => handleGuiConfigChange(config._uiId, 'tailwindFontSize', value)} disabled={isSaving}><SelectTrigger id={`el-twFontSize-${config._uiId}`} className="h-8 text-xs mt-0.5"><SelectValue placeholder="Select size" /></SelectTrigger><SelectContent>{TAILWIND_FONT_SIZES.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent></Select></div>
                                      <div><Label htmlFor={`el-twFontWeight-${config._uiId}`} className="text-xs">Font Weight (TW)</Label><Select value={config.tailwindFontWeight || NONE_VALUE} onValueChange={(value) => handleGuiConfigChange(config._uiId, 'tailwindFontWeight', value)} disabled={isSaving}><SelectTrigger id={`el-twFontWeight-${config._uiId}`} className="h-8 text-xs mt-0.5"><SelectValue placeholder="Select weight" /></SelectTrigger><SelectContent>{TAILWIND_FONT_WEIGHTS.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent></Select></div>
                                      <div><Label htmlFor={`el-twLineHeight-${config._uiId}`} className="text-xs">Line Height (TW)</Label><Select value={config.tailwindLineHeight || NONE_VALUE} onValueChange={(value) => handleGuiConfigChange(config._uiId, 'tailwindLineHeight', value)} disabled={isSaving}><SelectTrigger id={`el-twLineHeight-${config._uiId}`} className="h-8 text-xs mt-0.5"><SelectValue placeholder="Select line height" /></SelectTrigger><SelectContent>{TAILWIND_LINE_HEIGHTS.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent></Select></div>
                                ...
                                {(config.elementType === 'text' || config.elementType === 'textarea') && (
                                    <details className="space-y-2 group" open>
                                    <summary className="text-xs text-muted-foreground font-semibold cursor-pointer list-none flex items-center gap-1 group-open:mb-1.5"><Settings className="h-3 w-3 mr-1"/> Overflow & Display (TW) <ChevronRight className="h-3 w-3 group-open:rotate-90 transition-transform ml-auto" /></summary>
                                     <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pl-1">
                                ...
                                <summary className="text-xs text-muted-foreground font-semibold cursor-pointer list-none flex items-center gap-1 group-open:mb-1.5"><Settings className="h-3 w-3 mr-1"/> Borders <ChevronRight className="h-3 w-3 group-open:rotate-90 transition-transform ml-auto" /></summary>
                                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pl-1">
                                    <div><Label htmlFor={`el-twBorderRadius-${config._uiId}`} className="text-xs">Border Radius (TW)</Label><Select value={config.tailwindBorderRadius || NONE_VALUE} onValueChange={(value) => handleGuiConfigChange(config._uiId, 'tailwindBorderRadius', value)} disabled={isSaving}><SelectTrigger id={`el-twBorderRadius-${config._uiId}`} className="h-8 text-xs mt-0.5"><SelectValue /></SelectTrigger><SelectContent>{TAILWIND_BORDER_RADIUS_OPTIONS.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent></Select></div>
                                    <div><Label htmlFor={`el-twBorderColor-${config._uiId}`} className="text-xs">Border Color (TW)</Label><Select value={config.tailwindBorderColor || NONE_VALUE} onValueChange={(value) => handleGuiConfigChange(config._uiId, 'tailwindBorderColor', value)} disabled={isSaving}><SelectTrigger id={`el-twBorderColor-${config._uiId}`} className="h-8 text-xs mt-0.5"><SelectValue /></SelectTrigger><SelectContent>{TAILWIND_BORDER_COLOR_OPTIONS.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent></Select></div>
                                  </div>
        ...
        <Card className="md:w-[35%] sticky top-20 self-start shadow-lg">
          <CardHeader>
            ...
            <CardDescription className="text-sm">
              This preview updates as you modify the layout (either via GUI or JSON). Uses sample data based on field definitions.
            </CardDescription>
          </CardHeader>
