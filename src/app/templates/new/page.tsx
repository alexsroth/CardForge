...
export default function TemplateDesignerPage() {
  ...
  // State for GUI Layout Builder
  const [selectedSizePreset, setSelectedSizePreset] = useState<string>(
    COMMON_CARD_SIZES.find(s => s.width === defaultLayoutParsed.width && s.height === defaultLayoutParsed.height)?.value || COMMON_CARD_SIZES[0].value
  );
  ...
  // Sync `layoutElementGuiConfigs` with `fields` data structure changes
  useEffect(() => {
      ...
      fields.forEach((field, index) => {
        ...
        if (config) {
          ...
          newConfigs.push({
            ...
            tailwindBorderTopColor: NONE_VALUE,
            tailwindBorderRightW: NONE_VALUE,
            tailwindBorderRightColor: NONE_VALUE,
            tailwindBorderBottomW: NONE_VALUE,
            tailwindBorderBottomColor: NONE_VALUE,
            tailwindBorderLeftW: NONE_VALUE,
            tailwindBorderLeftColor: NONE_VALUE,
          });
        }
      });
  ...
  const handleGenerateJsonFromBuilder = useCallback((showSuccessToast = true) => {
    ...
    const generatedElements: LayoutElement[] = elementsToInclude.map(config => {
      ...
      if (sideBorderWidthClasses.length > 0 && sideBorderColorClasses.length === 0 && 
          (!config.tailwindBorderTopColor || config.tailwindBorderTopColor === NONE_VALUE) &&
          (!config.tailwindBorderRightColor || config.tailwindBorderRightColor === NONE_VALUE) &&
          (!config.tailwindBorderBottomColor || config.tailwindBorderBottomColor === NONE_VALUE) &&
          (!config.tailwindBorderLeftColor || config.tailwindBorderLeftColor === NONE_VALUE)) {
  ...
  const handleSaveTemplate = useCallback(async () => {
    ...
    setIsSaving(false);
  }, [templateName, fields, layoutDefinition, addTemplate, router, toast, existingTemplates, DEFAULT_CARD_LAYOUT_JSON_STRING]);
                        ...
                        onCheckedChange={(checked) => {
                            ...
                            if (newView === 'json' && activeEditorView === 'gui') {
                                ...
                                try {
                                    ...
                                    setCanvasBackgroundColor(String(parsedLayout.backgroundColor || defaultLayoutParsed.backgroundColor));
                                    setCanvasBorderColor(String(parsedLayout.borderColor || defaultLayoutParsed.borderColor));
                                    setCanvasBorderRadius(String(parsedLayout.borderRadius || defaultLayoutParsed.borderRadius));
                                    setCanvasBorderWidth(String(parsedLayout.borderWidth || defaultLayoutParsed.borderWidth));
                                    setCanvasBorderStyle(String(parsedLayout.borderStyle || defaultLayoutParsed.borderStyle));
                                    ...
                                    setLayoutElementGuiConfigs(currentFields => currentFields.map((fieldConfig, index) => {
                                        ...
                                        return {
                                          ...
                                          tailwindTextColor: existingJsonElement?.className?.split(' ').find(cls => TAILWIND_TEXT_COLORS.some(opt => opt.value !== NONE_VALUE && opt.value === cls)) || 'text-black',
                                          tailwindFontSize: existingJsonElement?.className?.split(' ').find(cls => TAILWIND_FONT_SIZES.some(opt => opt.value !== NONE_VALUE && opt.value === cls)) || NONE_VALUE,
                                          tailwindFontWeight: existingJsonElement?.className?.split(' ').find(cls => TAILWIND_FONT_WEIGHTS.some(opt => opt.value !== NONE_VALUE && opt.value === cls)) || NONE_VALUE,
                                          tailwindLineHeight: existingJsonElement?.className?.split(' ').find(cls => TAILWIND_LINE_HEIGHTS.some(opt => opt.value !== NONE_VALUE && opt.value === cls)) || NONE_VALUE,
                                          tailwindOverflow: existingJsonElement?.className?.split(' ').find(cls => TAILWIND_OVERFLOW.some(opt => opt.value !== NONE_VALUE && opt.value === cls)) || NONE_VALUE,
                                          tailwindTextOverflow: existingJsonElement?.className?.split(' ').find(cls => TAILWIND_TEXT_OVERFLOW.some(opt => opt.value !== NONE_VALUE && opt.value === cls)) || NONE_VALUE,
                                          
                                          tailwindBorderRadius: existingJsonElement?.className?.split(' ').find(cls => TAILWIND_BORDER_RADIUS_OPTIONS.some(opt => opt.value !== NONE_VALUE && opt.value === cls)) || NONE_VALUE,
                                          tailwindBorderTopW: BORDER_SIDE_WIDTH_OPTIONS.find(opt => opt.value !== NONE_VALUE && getSideBorderWidthClass('t', opt.value) && existingJsonElement?.className?.includes(getSideBorderWidthClass('t', opt.value)))?.value || NONE_VALUE,
                                          tailwindBorderRightW: BORDER_SIDE_WIDTH_OPTIONS.find(opt => opt.value !== NONE_VALUE && getSideBorderWidthClass('r', opt.value) && existingJsonElement?.className?.includes(getSideBorderWidthClass('r', opt.value)))?.value || NONE_VALUE,
                                          tailwindBorderBottomW: BORDER_SIDE_WIDTH_OPTIONS.find(opt => opt.value !== NONE_VALUE && getSideBorderWidthClass('b', opt.value) && existingJsonElement?.className?.includes(getSideBorderWidthClass('b', opt.value)))?.value || NONE_VALUE,
                                          tailwindBorderLeftW: BORDER_SIDE_WIDTH_OPTIONS.find(opt => opt.value !== NONE_VALUE && getSideBorderWidthClass('l', opt.value) && existingJsonElement?.className?.includes(getSideBorderWidthClass('l', opt.value)))?.value || NONE_VALUE,
                                          
                                          tailwindBorderTopColor: TAILWIND_BORDER_PALETTE_OPTIONS.find(opt => opt.value !== NONE_VALUE && getSideBorderColorClass('t', opt.value) && existingJsonElement?.className?.includes(getSideBorderColorClass('t', opt.value)))?.value || NONE_VALUE,
                                          tailwindBorderRightColor: TAILWIND_BORDER_PALETTE_OPTIONS.find(opt => opt.value !== NONE_VALUE && getSideBorderColorClass('r', opt.value) && existingJsonElement?.className?.includes(getSideBorderColorClass('r', opt.value)))?.value || NONE_VALUE,
                                          tailwindBorderBottomColor: TAILWIND_BORDER_PALETTE_OPTIONS.find(opt => opt.value !== NONE_VALUE && getSideBorderColorClass('b', opt.value) && existingJsonElement?.className?.includes(getSideBorderColorClass('b', opt.value)))?.value || NONE_VALUE,
                                          tailwindBorderLeftColor: TAILWIND_BORDER_PALETTE_OPTIONS.find(opt => opt.value !== NONE_VALUE && getSideBorderColorClass('l', opt.value) && existingJsonElement?.className?.includes(getSideBorderColorClass('l', opt.value)))?.value || NONE_VALUE,
                                        };
                                    }));
                                    setLayoutJsonError(null);
                                ...
                                <details className="space-y-1.5 group" open>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-2 pl-1">
                                        ...
                                        {config.elementType === 'iconValue' && ( <div className="space-y-1"> <Label htmlFor={`el-icon-new-${config._uiId}`} className="text-xs">Icon Name (Lucide)</Label> <Input id={`el-icon-new-${config._uiId}`} value={config.iconName || ''} onChange={(e) => handleGuiConfigChange(config._uiId, 'iconName', e.target.value)} placeholder="e.g., Coins" className="h-8 text-xs mt-0.5" disabled={isSaving}/>
                                            <Accordion type="single" collapsible className="w-full text-xs" defaultValue={`icon-browser-inline-new-${config._uiId}-item`}>
                                            <AccordionItem value={`icon-browser-inline-new-${config._uiId}-item`} className="border-b-0">
            ...
            {activeEditorView === 'json' && (
              <div className="mt-4 flex-grow flex flex-col">
                  <div>
                    ...
                    <Textarea id="layoutDefinition" value={layoutDefinition} onChange={handleLayoutDefinitionChangeFromTextarea} onBlur={validateAndFormatLayoutJson} placeholder="Click \"Generate/Update JSON from Builder\" (in page actions menu) to populate, or paste/edit your JSON here." rows={15} className="font-mono text-xs flex-grow min-h-[200px] max-h-[350px] bg-muted/20 mt-1" disabled={isSaving} />
                  </div>
