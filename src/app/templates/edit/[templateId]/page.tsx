...
const TAILWIND_BACKGROUND_COLORS: Array<{value: string, label: string}> = [
    { value: NONE_VALUE, label: "None (Use Direct CSS Input or Transparent)"},
    { value: "bg-transparent", label: "Transparent" },
    { value: "bg-card", label: "Default Card BG (Theme)"},
    { value: "bg-background", label: "Default Page BG (Theme)"},
    { value: "bg-primary", label: "Primary (Theme)"},
    { value: "bg-secondary", label: "Secondary (Theme)"},
    { value: "bg-muted", label: "Muted (Theme)"},
    { value: "bg-white", label: "White"},
    { value: "bg-black", label: "Black"},
    { value: "bg-slate-500", label: "Slate 500" }, { value: "bg-red-500", label: "Red 500" },
    { value: "bg-orange-500", label: "Orange 500" }, { value: "bg-amber-500", label: "Amber 500" },
    { value: "bg-yellow-500", label: "Yellow 500" }, { value: "bg-lime-500", label: "Lime 500" },
    { value: "bg-green-500", label: "Green 500" }, { value: "bg-emerald-500", label: "Emerald 500" },
    { value: "bg-teal-500", label: "Teal 500" }, { value: "bg-cyan-500", label: "Cyan 500" },
    { value: "bg-sky-500", label: "Sky 500" }, { value: "bg-blue-500", label: "Blue 500" },
    { value: "bg-indigo-500", label: "Indigo 500" }, { value: "bg-violet-500", label: "Violet 500" },
...
function mapTemplateFieldToFieldDefinition(field: TemplateField, index: number): TemplateFieldDefinition {
  return {
    _uiId: `field-edit-${field.key}-${Date.now()}-${index}`, // Ensure a unique ID for edit page
    key: field.key,
    label: field.label,
    type: field.type,
    placeholder: field.placeholder,
    defaultValue: field.defaultValue,
    previewValue: field.defaultValue !== undefined ? String(field.defaultValue) : '', // Default preview to default value
...
export default function EditTemplatePage() {
    ...
    if (templateToEdit) {
      ...
      try {
        ...
        if (parsedLayout.canvasClassName) {
            ...
            const bwClass = classes.find(c => BORDER_SIDE_WIDTH_OPTIONS.some(opt => opt.value !== NONE_VALUE && opt.value === c && (c.startsWith("border-") && !c.startsWith("border-t") && !c.startsWith("border-r") && !c.startsWith("border-b") && !c.startsWith("border-l")) ) ) || NONE_VALUE;
            setTailwindCanvasBorderWidth(bwClass);
        ...
        setLayoutElementGuiConfigs(initialFields.map((field, index) => {
          ...
          const config: LayoutElementGuiConfig = {
            ...
            // styleWidth: existingLayoutElement?.style?.width || '120px', // Removed
            // styleHeight: existingLayoutElement?.style?.height || '20px', // Removed
            ...
            styleFontSize: existingLayoutElement?.style?.fontSize || '12px', // Keep for direct CSS fontSize if needed
            // styleFontWeight: existingLayoutElement?.style?.fontWeight || '', // Removed for Tailwind
            ...
            // styleBorderTop: existingLayoutElement?.style?.borderTop || '', // Removed for Tailwind
            // styleBorderBottom: existingLayoutElement?.style?.borderBottom || '', // Removed for Tailwind
            // styleOverflow: existingLayoutElement?.style?.overflow || 'visible', // Removed for Tailwind
            // styleTextOverflow: existingLayoutElement?.style?.textOverflow || 'clip', // Removed for Tailwind
            iconName: existingLayoutElement?.icon || (field.type === 'number' ? 'Coins' : ''),
            
            tailwindTextColor: NONE_VALUE, tailwindFontSize: 'text-base', tailwindFontWeight: 'font-normal',
            ...
            // tailwindBorderWidth: NONE_VALUE, // Removed global, use per-side
          ...
          if (existingLayoutElement?.className) {
            ...
            config.tailwindTextColor = classes.find(c => TAILWIND_TEXT_COLORS.some(tc => tc.value === c && c !== NONE_VALUE)) || config.tailwindTextColor;
            config.tailwindFontSize = classes.find(c => TAILWIND_FONT_SIZES.some(ts => ts.value === c && c !== NONE_VALUE)) || config.tailwindFontSize;
            config.tailwindFontWeight = classes.find(c => TAILWIND_FONT_WEIGHTS.some(tw => tw.value === c && c !== NONE_VALUE)) || config.tailwindFontWeight;
            config.tailwindLineHeight = classes.find(c => TAILWIND_LINE_HEIGHTS.some(tl => tl.value === c && c !== NONE_VALUE)) || config.tailwindLineHeight;
            config.tailwindOverflow = classes.find(c => TAILWIND_OVERFLOW.some(to => to.value === c && c !== NONE_VALUE)) || config.tailwindOverflow;
            config.tailwindTextOverflow = classes.find(c => TAILWIND_TEXT_OVERFLOW.some(tto => tto.value === c && c !== NONE_VALUE)) || config.tailwindTextOverflow;
            config.tailwindBorderRadius = classes.find(c => TAILWIND_BORDER_RADIUS.some(br => br.value === c && c !== NONE_VALUE)) || config.tailwindBorderRadius;
            config.tailwindBorderColor = classes.find(c => TAILWIND_BORDER_COLORS.some(bc => bc.value === c && c !== NONE_VALUE)) || config.tailwindBorderColor;

            // Parse per-side border widths
            config.tailwindBorderTopW = classes.find(c => c.startsWith('border-t-') && BORDER_SIDE_WIDTH_OPTIONS.some(opt => getSideBorderWidthClass('t', opt.value === NONE_VALUE ? '' : opt.value) === c)) || classes.find(c => c === 'border-t' && BORDER_SIDE_WIDTH_OPTIONS.some(opt => opt.value === 'default')) || NONE_VALUE;
            config.tailwindBorderRightW = classes.find(c => c.startsWith('border-r-') && BORDER_SIDE_WIDTH_OPTIONS.some(opt => getSideBorderWidthClass('r', opt.value === NONE_VALUE ? '' : opt.value) === c)) || classes.find(c => c === 'border-r' && BORDER_SIDE_WIDTH_OPTIONS.some(opt => opt.value === 'default')) || NONE_VALUE;
            config.tailwindBorderBottomW = classes.find(c => c.startsWith('border-b-') && BORDER_SIDE_WIDTH_OPTIONS.some(opt => getSideBorderWidthClass('b', opt.value === NONE_VALUE ? '' : opt.value) === c)) || classes.find(c => c === 'border-b' && BORDER_SIDE_WIDTH_OPTIONS.some(opt => opt.value === 'default')) || NONE_VALUE;
            config.tailwindBorderLeftW = classes.find(c => c.startsWith('border-l-') && BORDER_SIDE_WIDTH_OPTIONS.some(opt => getSideBorderWidthClass('l', opt.value === NONE_VALUE ? '' : opt.value) === c)) || classes.find(c => c === 'border-l' && BORDER_SIDE_WIDTH_OPTIONS.some(opt => opt.value === 'default')) || NONE_VALUE;
        ...
        setLayoutElementGuiConfigs(initialFields.map((field, index) => {
            ...
            return {
                ...
                // styleWidth: '120px', styleHeight: '20px',
                ...
                // styleOverflow: 'visible', styleTextOverflow: 'clip',
                tailwindTextColor: NONE_VALUE, tailwindFontSize: 'text-base', tailwindFontWeight: 'font-normal',
                tailwindLineHeight: 'leading-normal', tailwindOverflow: 'overflow-visible', tailwindTextOverflow: NONE_VALUE,
  ...
  useEffect(() => {setLayoutElementGuiConfigs(prevConfigs => {
      ...
      fields.forEach((field, index) => {
        ...
        if (existingConfig) {
          ...
          const currentTop = yOffsetBase + (index * yIncrement);
          finalConfigs.push({
            ...
            isExpandedInGui: false,
            elementType: field.type === 'textarea' ? 'textarea' : (field.type === 'placeholderImage' ? 'image' : 'text'),
            styleTop: `${currentTop}px`,
            styleLeft: `10px`,
            styleRight: '', styleBottom: '',
            // styleWidth: `calc(100% - 20px)`, styleHeight: field.type === 'textarea' ? `60px` : `20px`,
            styleMaxHeight: field.type === 'textarea' ? '80px' : (field.type === 'placeholderImage' ? '140px' : (field.type === 'number' ? 'auto' : 'auto')), 
            styleFontSize: '12px', 
            // styleFontWeight: '',
            iconName: field.type === 'number' ? 'Coins' : '',
  ...
  useEffect(() => {console.log('[DEBUG] EditTemplatePage: Generating sampleCardForPreview. Fields count:', fields.length, 'Original ID:', originalTemplateId);
  ...
  const handleGenerateJsonFromBuilder = () => {
    ...
    const generatedElements = elementsToInclude.map(config => {
      ...
      if (config.elementType === 'text' || config.elementType === 'textarea' || config.elementType === 'iconValue') {
        if (config.styleFontSize?.trim()) style.fontSize = config.styleFontSize.trim().endsWith('px') || config.styleFontSize.trim().endsWith('em') ? config.styleFontSize.trim() : `${config.styleFontSize.trim()}px`;
        // style.fontWeight is now handled by Tailwind
        ...
        // style.lineHeight is now handled by Tailwind
      }
      
      if (config.stylePadding?.trim()) style.padding = config.stylePadding.trim();
      // style.borderTop and style.borderBottom are now handled by Tailwind per-side borders
      // style.overflow and style.textOverflow are now handled by Tailwind
      
      const classNames = [];
      if (config.originalType === 'textarea' || config.elementType === 'textarea') classNames.push('whitespace-pre-wrap');
      
      if (config.elementType === 'text' || config.elementType === 'textarea' || config.elementType === 'iconValue') {
        if (config.tailwindTextColor && config.tailwindTextColor !== NONE_VALUE) classNames.push(config.tailwindTextColor); else classNames.push('text-card-foreground');
        if (config.tailwindFontSize && config.tailwindFontSize !== NONE_VALUE) classNames.push(config.tailwindFontSize); else classNames.push('text-base');
        if (config.tailwindFontWeight && config.tailwindFontWeight !== NONE_VALUE) classNames.push(config.tailwindFontWeight); else classNames.push('font-normal');
        if (config.tailwindLineHeight && config.tailwindLineHeight !== NONE_VALUE) classNames.push(config.tailwindLineHeight); else classNames.push('leading-normal');
        if (config.tailwindOverflow && config.tailwindOverflow !== NONE_VALUE) classNames.push(config.tailwindOverflow); else classNames.push('overflow-visible');
      ...
      ].filter(w => w && w !== NONE_VALUE && w.startsWith('border-'));
  ...
  const handleSaveTemplate = async () => {
    ...
    if (!finalLayoutDefToSave) {
       if (layoutElementGuiConfigs.some(c => c.isEnabledOnCanvas)) {
            ...
            finalLayoutDefToSave = layoutDefinition;
             if (!finalLayoutDefToSave.trim() || JSON.parse(finalLayoutDefToSave).elements.length === 0) {
                finalLayoutDefToSave = DEFAULT_CARD_LAYOUT_JSON_STRING;
                setLayoutDefinition(finalLayoutDefToSave);
             }
        } else {
            finalLayoutDefToSave = DEFAULT_CARD_LAYOUT_JSON_STRING;
            setLayoutDefinition(finalLayoutDefToSave);
      ...
      try {
        const parsed = JSON.parse(finalLayoutDefToSave);
         if (!parsed.elements || !Array.isArray(parsed.elements) || (parsed.elements.length === 0 && layoutElementGuiConfigs.some(c => c.isEnabledOnCanvas))) {
            if (layoutElementGuiConfigs.some(c => c.isEnabledOnCanvas)) {
                 handleGenerateJsonFromBuilder();
                 finalLayoutDefToSave = layoutDefinition; 
        ...
        finalLayoutDefToSave = JSON.stringify(JSON.parse(finalLayoutDefToSave), null, 2);
  ...
  const handleCanvasTailwindPropertyChange = (
    prop: keyof Pick<EditTemplatePage['state'], 'tailwindCanvasBackgroundColor' | 'tailwindCanvasBorderRadius' | 'tailwindCanvasBorderWidth' | 'tailwindCanvasBorderColor'>,
    value: string
  ) => {
    console.log('[DEBUG] EditTemplatePage/handleCanvasTailwindPropertyChange:', prop, value);
     switch(prop) {
          ...
          <CardContent className="flex-grow space-y-4 flex flex-col">
            ...
            <div className="space-y-3 p-3 border rounded-md bg-muted/30">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3 items-end mt-2">
                ...
                <div>
                 ...
                 <div>
                  <Label htmlFor="tailwindCanvasBorderWidthEdit" className="text-xs font-medium">Border Width (Tailwind)</Label>
                   <Select value={tailwindCanvasBorderWidth} onValueChange={(value) => handleCanvasTailwindPropertyChange('tailwindCanvasBorderWidth', value)} disabled={isSaving}>
                    <SelectTrigger id="tailwindCanvasBorderWidthEdit" className="mt-1 h-8 text-xs"><SelectValue /></SelectTrigger>
            ...
            <div className="space-y-3 p-3 border rounded-md bg-muted/30">
              <h4 className="text-base font-semibold mb-1">Layout Elements (Toggle to Include & Configure)</h4>
               <ScrollArea className="pr-2"> {/* Removed max-h for expansion */}
                  <div className="space-y-2">
                           ...
                           <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pl-1">
                            ...
                            {config.elementType === 'iconValue' && (
                              <div className="space-y-1">
                                ...
                                <Accordion type="single" collapsible className="w-full text-xs">
                                  <AccordionItem value="icon-browser-inline-edit" className="border-b-0">
                                    ...
                                    <AccordionContent className="p-2 border rounded bg-muted/50 max-h-[150px] overflow-y-auto">
                                        ...
                                        <ScrollArea className="max-h-[120px] bg-background/50 p-1 rounded border">
                                            <div className={cn("grid gap-0.5", "grid-cols-8 sm:grid-cols-10 md:grid-cols-12")}>
                                            {commonLucideIconsForGuide.map(iconKey => (
                                                <TooltipProvider key={`${iconKey}-edit`} delayDuration={100}>
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                    <Button variant="ghost" size="icon" onClick={() => handleCopyIconName(iconKey)} className="h-6 w-6 p-0.5" >
                                                        <IconComponent name={iconKey} className="h-3.5 w-3.5" />
                                                    </Button>
                                                    </TooltipTrigger>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pl-1">
                                {['styleTop', 'styleLeft', 'styleRight', 'styleBottom'].map(prop => (
                                    <div key={prop}>
                                        <Label htmlFor={`el-${prop}-edit-${config._uiId}`} className="text-xs capitalize">{prop.replace('style', '').replace(/([A-Z])/g, ' $1').trim()} (CSS value)</Label>
                                 ...
                                 <div>
                                      <Label htmlFor={`el-styleMaxHeight-edit-${config._uiId}`} className="text-xs">Max Height (CSS)</Label>
                                      <Input id={`el-styleMaxHeight-edit-${config._uiId}`} value={config.styleMaxHeight || ''} onChange={(e) => handleGuiConfigChange(config._uiId, 'styleMaxHeight', e.target.value)} className="h-8 text-xs mt-0.5" placeholder="e.g., 80px" disabled={isSaving}/>
                                  </div>
                                  <div>
                                      <Label htmlFor={`el-stylePadding-edit-${config._uiId}`} className="text-xs">Padding (CSS)</Label>
                                      <Input id={`el-stylePadding-edit-${config._uiId}`} value={config.stylePadding || ''} onChange={(e) => handleGuiConfigChange(config._uiId, 'stylePadding', e.target.value)} className="h-8 text-xs mt-0.5" placeholder="e.g., 5px or 2px 4px" disabled={isSaving}/>
                                  </div>
                            </div>
                            </details>
            ...
            <div className="mt-4 flex-grow flex flex-col min-h-0">
              ...
              {/* Top-level Icon Browser Accordion Removed */}
            </div>
          </CardContent>
