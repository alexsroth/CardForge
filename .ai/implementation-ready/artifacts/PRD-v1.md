...
3. Two-template model is required: Card Data Template and Layout Template.
4. Card data field types in V1 are `category` (stable option IDs),
   `singleLineText`, `areaText`, `number`, `image` (dimension guidance), and
   `icon` (optional category mapping).
5. Layout authoring is container-first: draw/place container first, map data
   field second.
6. V1 layout elements are single line text, text with icon, area text, image,
   layout/group container, and decoration (borders/boxes/outlines).
...
4. Print canvas configuration includes unit (`mm` or `in`), width/height, DPI,
   bleed margins, safe-zone margins, and optional trim/corner settings.
...
4. User must choose one resolution: bulk migrate affected cards with field
   remap, or remove affected cards.
...
2. V1 bulk operations include CSV import and bulk template switch with required
   remap.
3. Mixed-template selection requires remap per source template group.
4. CSV workflow must support template CSV export, column mapping at import,
   invalid row skipping, and an import issues report.
5. Save model includes autosave, named checkpoints, and rollback to checkpoint.
...
3. V1 primitive set is text, image slot, icon, stat value, and shape/container.
...
1. Full export includes game metadata, data templates used by the game, layout
   templates used by the game, decks/cards, and assets.
2. Partial exports include cards only, templates only, and full game.
3. Export can target current state or a selected named checkpoint.
...
1. Hybrid persistence uses SQLite for operational data, JSON contracts for
   portability, and managed assets on disk.
